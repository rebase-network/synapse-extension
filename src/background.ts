...
chrome.runtime.onMessage.addListener(async (request, sender, sendResponse) => {
  ...
  if (request.messageType === MESSAGE_TYPE.IMPORT_MNEMONIC) {
    ...
    const entropyKeystore = Keystore.encrypt(
    ...
    const rootKeystore = Keystore.encrypt(
    ...
    const addressesObj = findInAddressesListByPublicKey(
      publicKey,
      addressesList
    );
  ...
  if (request.messageType === "xxxx") {
    const tx = {
      hash:
        "0x670133ec69d03de5f76f320b38c434cb474620d961c0dcecc5b73f64c3755947",
      block_num: 190532,
      ...
      for (let index = 0; index < addressesListObj.length; index++) {
        ...
        for (let index2 = 0; index2 < addresses.length; index2++) {
          ...
          const addressBack =
            address.substr(0, 16) +
            "..." +
            address.substr(address.length - 16, address.length);
...
function saveToCurrentWallet(currentWallet) {
  chrome.storage.sync.set({ currentWallet }, () => {
    console.log(
      "currentWallet is set to storage: " + JSON.stringify(currentWallet)
    );
  });
}
...
function saveToStorage() {
  ...
  chrome.storage.sync.set({ addressesList }, () => {
    console.log(
      "addressesList is set to storage: " + JSON.stringify(addressesList)
    );
  });
}
